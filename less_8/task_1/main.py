...
import re  # модуль поддержки регулярных выражений
import csv


def get_data():
    list_file = ['info_1.txt', 'info_2.txt', 'info_3.txt']  # файлы для парсинга
    # создание шаблона для парсинга при помощи регулярных выражений
    list_template = [
    ...
    # создаем списки по категориям
    os_prod_list = []
    os_name_list = []
    os_code_list = []
    os_type_list = []
    for i in list_file:  # открываем на чтение файлы по очереди
        with open(i, encoding='utf-8') as file_in:
            input = file_in.read()
            # заполняем списки категории результатами парсинга
...
def write_to_csv(file_name):
    with open(file_name, 'w', encoding="utf-8") as f_t:  # создаем и открываем файл csv на запись
        ...
        data = get_data()
        for i in data:
            wrtie_csv.writerow(i)  # записываем файл построчно


write_to_csv('homework_8.1.csv')
